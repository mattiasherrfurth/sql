...
--            And qnot_orop_no = orop_no
      ...
      WHERE
         ILOR_END_DT >= TO_DATE('2018-01-01', 'YYYY-MM-DD') And ILOR_END_DT <= sysdate  --This date is 1 of 2 locations that time range must be changed
         ...
         And Not (wctr_wcct_id = 'Z001' And Not wctr_cd In ('MY1FLEXA','MY1FLEXB','MYACTFIN','MYJUICE'))    --Manufacturing work centers that have interesting yield results (most mfg work centers don't)
         --And ilor_ordr_rtg_no = '0005521396'                                     --(for testing) Includes only order/s specified
         --And ilor_part_no = 'NP289K137G03      '                                         --(for testing) Includes only part number/s specified
...
/*------Begin section to bring in part groups through ORAD link----------------
--     Left Join (                                                                 --START subquery to get management overview groups from EMDD  (this will bring in EMDD part groupings if being run in oracle with a user ID with EMDD access)
--            select 
...
--            where is_active = 1
--            ) mgmtPartGrp On                                                      --END subquery to get management overview groups from EMDD       
...
---------End section to bring in part groups through ORAD link---------------*/		
WHERE
   ilor_end_dt >= To_date('2018-01-01', 'YYYY-MM-DD') And ilor_end_dt <= Sysdate  --This date is 1 of 2 locations that time range must be changed
