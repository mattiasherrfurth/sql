...
FROM
  ...
  Left Join(
    select 
      ...
      sum(orop_completed_qty) as pn_ops_per_day
    from tdwhorop
    inner join tdwhordr on ordr_rtg_no = orop_ordr_rtg_no
    where 
      rownum < 1000
    group by 
      ordr_no,
  ...
  on qnot_part_no = ordr_part_no and qnot_created_dt = orop_act_completed_dtm
